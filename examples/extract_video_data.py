...
from TikTok videos using nodriver (async CDP).
"""
...
async def extract_video_data(video_url, tab):
    """
    ...
    try:
        ...
        logger.error(f"Error extracting video data: {e}", exc_info=True)
        return {
            'extraction_timestamp': datetime.now().isoformat(),
            'video_url': video_url,
            'error': str(e),
...
async def main():
    ...
    try:
        ...
        # Extract video data
        video_data = await extract_video_data(video_url, tab)

        # Save data to file
        if video_data.get('success', True):
            ...
            if filename:
                ...
                if stats:
                    try:
                        ...
                        print(f"   Engagement: {stats.get('diggCount', 0)} likes, {stats.get('commentCount', 0)} comments")
